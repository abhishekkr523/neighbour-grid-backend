...
import { FileDisputeDto, ResolveDisputeDto } from "./disputes.types";
...
export class DisputesService {
  ...
  async fileDispute(filedBy: string, dto: FileDisputeDto) {
    ...
    try {
      ...
      const resCheck = await client.query(
        `SELECT r.id, r.status, r.borrower_id, r.end_date, t.owner_id
         FROM reservations r
         JOIN tools t ON t.id = r.tool_id
         WHERE r.id = $1 AND t.owner_id = $2`,
        [dto.reservation_id, filedBy]
      );

      if (resCheck.rows.length === 0) {
        throw new Error(
          "Reservation not found or you are not the tool owner"
        );
      }
      ...
      if (reservation.status !== "RETURNED") {
        throw new Error(
          `Disputes can only be filed when reservation is RETURNED (current: ${reservation.status})`
        );
      }
      ...
      if (hoursSinceReturn > 48) {
        throw new Error(
          "Dispute filing window has expired (must be within 24 hours of return)"
        );
      }

      // Create dispute record
      const disputeResult = await client.query(
        `INSERT INTO disputes (reservation_id, filed_by, reason, evidence_urls)
         VALUES ($1, $2, $3, $4)
  ...
  async getDisputeById(disputeId: string) {
    ...
    const auditsResult = await pool.query(
      `SELECT id, uploaded_by, audit_type, image_url, image_category, notes, created_at
       FROM audits
       WHERE reservation_id = $1
       ORDER BY audit_type ASC, image_category ASC`,
      [dispute.reservation_id]
    );

    const pickup = auditsResult.rows.filter(
      (r: any) => r.audit_type === "PICKUP"
    );
    const returnAudit = auditsResult.rows.filter(
  ...
  async resolveDispute(
    ...
    try {
      ...
      const disputeResult = await client.query(
        ...
        [dto.resolution, dto.admin_notes || null, adminId, disputeId]
      );
      ...
      if (dto.resolution === "RESOLVED_BORROWER") {
        ...
        console.log(
          `[Dispute] Resolved in owner's favor — deposit retained for reservation ${dispute.reservation_id}`
        );
      }
