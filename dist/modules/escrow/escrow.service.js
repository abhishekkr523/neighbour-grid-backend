...
const escrowSettlementQueue = new bullmq_1.Queue("escrow-settlement", {
...
class StripeStub {
    ...
    async createPaymentIntent(amountCents, _currency = "usd") {
        const intentId = `pi_stub_${Date.now()}_${Math.random().toString(36).substring(7)}`;
        console.log(`[Stripe Stub] Created PaymentIntent ${intentId} for ${amountCents} cents (capture_method: manual)`);
        return {
            id: intentId,
    ...
    async capturePaymentIntent(intentId, amountToCapture) {
        console.log(`[Stripe Stub] Captured ${amountToCapture} cents on ${intentId}`);
        return {
            id: intentId,
            status: "succeeded",
...
class EscrowService {
    ...
    async createPaymentHold(reservationId, userId) {
        const client = await db_1.pool.connect();
        try {
            await client.query("BEGIN");
            // Fetch reservation (must be CONFIRMED and belong to borrower)
            const resResult = await client.query(`SELECT r.id, r.status, r.total_fee_cents, r.deposit_cents, r.borrower_id
         FROM reservations r
         WHERE r.id = $1 AND r.borrower_id = $2`, [reservationId, userId]);
            if (resResult.rows.length === 0) {
                throw new Error("Reservation not found or not your booking");
            }
            const reservation = resResult.rows[0];
            if (reservation.status !== "CONFIRMED") {
                throw new Error(`Reservation must be in CONFIRMED status (current: ${reservation.status})`);
            }
            // Total hold = rental fee + security deposit
            const totalHoldAmount = reservation.total_fee_cents + reservation.deposit_cents;
         ...
         WHERE id = $2`, [intent.id, reservationId]);
    ...
    async settleEscrow(reservationId) {
        const client = await db_1.pool.connect();
        try {
            await client.query("BEGIN");
            const resResult = await client.query(`SELECT id, status, total_fee_cents, deposit_cents, stripe_payment_intent_id
         FROM reservations WHERE id = $1`, [reservationId]);
            ...
            if (reservation.status !== "RETURNED") {
                throw new Error(`Reservation must be in RETURNED status for settlement (current: ${reservation.status})`);
            }
            ...
            await stripe.capturePaymentIntent(reservation.stripe_payment_intent_id, rentalFee);
            // Release security deposit hold
            // (In real Stripe, you'd handle this with separate intents or partial capture)
            console.log(`[Escrow] Releasing deposit of ${deposit} cents for reservation ${reservationId}`);
    ...
    async freezeDeposit(reservationId) {
        const result = await db_1.pool.query(`UPDATE reservations SET status = 'DISPUTED', updated_at = NOW()
       WHERE id = $1 AND status = 'RETURNED'
       RETURNING id, status`, [reservationId]);
        if (result.rows.length === 0) {
            throw new Error("Reservation not in RETURNED status");
        }
        console.log(`[Escrow] Deposit frozen for reservation ${reservationId} — awaiting admin resolution`);
        return result.rows[0];
    }
    // ─── ENQUEUE AUTO-SETTLEMENT (24h after return) ────────────
    async enqueueAutoSettlement(reservationId) {
        await escrowSettlementQueue.add("auto-settle", { reservationId }, {
            delay: 24 * 60 * 60 * 1000, // 24 hours
            attempts: 3,
            backoff: { type: "exponential", delay: 5000 },
        });
        console.log(`[Escrow] Auto-settlement enqueued for reservation ${reservationId} (24h delay)`);
    }
}
